...
def compute_metrics(journeys: pl.DataFrame) -> dict:
    ...
    converting = journeys.filter(pl.col("has_conversion") == True)
    non_converting = journeys.filter(pl.col("has_conversion") == False)
    ...
    conversion_rate = (conversion_users / total_users * 100) if total_users > 0 else 0
    avg_revenue = total_revenue / conversion_users if conversion_users > 0 else 0
